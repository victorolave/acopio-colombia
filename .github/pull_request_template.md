...
- [ ] Ejecuté `pnpm run geocode` y **revisé a qué lugar resolvió** cada dirección nueva
- [ ] `location_precision` refleja la realidad; ante la duda usé el valor más bajo
- [ ] Ejecuté `pnpm run validate:seed` y pasa
- [ ] Ejecuté `pnpm run seed:build` e incluí `supabase/seed.sql` en el commit
